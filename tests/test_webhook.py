...
from frontdesk_whatsapp.protocol import buttons_message, can_send_free_form, list_message, signature, template_message
...
def payload(phone,body,owner="15550001111",wamid=None,kind="text"):
    message={"from":owner,"id":wamid or uuid7(),"timestamp":str(int(now().timestamp())),"type":kind}
    if kind=="text":
        message["text"]={"body":body}
    else:
        message["interactive"]={"type":"list_reply","list_reply":{"id":body,"title":"Choice"}}
    return {"object":"whatsapp_business_account","entry":[{"id":"demo-account","changes":[{"field":"messages","value":{"messaging_product":"whatsapp","metadata":{"phone_number_id":phone},"messages":[message]}}]}]}
...
async def send(client,phone,body,owner="15550001111",wamid=None,kind="text"):
    ...
    return await client.post("/webhooks/whatsapp",content=raw,headers={"X-Hub-Signature-256":signature(raw,"local-meta-secret")})
...
async def test_webhook_replay_and_restart(db,clinic):
    ...
    with Session(db) as session:
        ...
        messages=session.exec(select(Message).where(Message.clinic_id==c.id,Message.wamid==wamid)).all()
        assert len(bookings)==len(messages)==1
    evidence(db,"idempotency",{"deliveries":2,"booking_rows":len(bookings),"inbound_rows":len(messages),"additional_outbound_on_retry":len(transport.calls)-before,"restart_resumed":True})

async def test_signature_rejects_before_parse(db,clinic):
    c,*_=clinic
    async with httpx.AsyncClient(transport=httpx.ASGITransport(app=create_app(db=db)),base_url="http://test") as client:
        raw=json.dumps(payload(c.whatsapp_phone_number_id,"hello")).encode()
        response=await client.post("/webhooks/whatsapp",content=raw+b"x",headers={"X-Hub-Signature-256":signature(raw,"local-meta-secret")})
        assert response.status_code==403
        invalid=await client.post("/webhooks/whatsapp",content=b"invalid",headers={"X-Hub-Signature-256":signature(b"invalid","local-meta-secret")})
        assert invalid.status_code==400
        assert (await client.get("/webhooks/whatsapp",params={"hub.mode":"subscribe","hub.verify_token":"local-verify-token","hub.challenge":"123"})).text=="123"
...
@pytest.mark.parametrize("seconds,expected",[(86399,True),(86400,False),(86401,False),(-1,False),(0,True)])
...
async def test_simulator_identity_and_ops_auth(db):
    ...
    with Session(db) as session:
        ...
        existing=session.exec(select(Clinic).where(Clinic.whatsapp_phone_number_id=="demo-phone")).first()
        if existing is None:
            make_clinic(session,"demo-phone")
    async with httpx.AsyncClient(transport=httpx.ASGITransport(app=create_app(db=db)),base_url="http://test") as client:
        identity=(await client.post("/internal/simulator/session")).json()
        headers={"Authorization":"Bearer "+identity["token"]}
        good=await client.post("/internal/simulator",json=payload("demo-phone","hello",identity["wa_id"]),headers=headers)
        assert good.status_code==200
        assert (await client.post("/internal/simulator",json=payload("demo-phone","hello","15550001111"),headers=headers)).status_code==403
        assert (await client.post("/internal/simulator",json={})).status_code==401
        assert (await client.get("/ops")).status_code==401
        data=(await client.get("/ops",headers={"Authorization":"Bearer local-ops-token"})).json()
        cid=good.json()["results"][0]["conversation_id"]
        replay=await client.get("/ops/replay/"+cid,params={"clinic_id":data["clinic"]["id"]},headers={"Authorization":"Bearer local-ops-token"})
        assert replay.status_code==200 and replay.json()["messages"]
        assert (await client.get("/ops/replay/"+cid,params={"clinic_id":"foreign"},headers={"Authorization":"Bearer local-ops-token"})).status_code==403
