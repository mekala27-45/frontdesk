...
def test_meta_transport_contract():
    ...
    body={"messaging_product":"whatsapp","to":"15550001111","type":"text","text":{"body":"Logistics only."}}
...
def test_google_insert_update_delete(clinic):
    ...
    booking=Booking(clinic_id=c.id,slot_id=slots[0].id,patient_wa_id="15550001111",service_id=slots[0].service_id)
    service=MagicMock()
    ...
    service.events().insert().execute.side_effect=HttpError(SimpleNamespace(status=409,reason="Conflict"),b"{}")
    assert adapter.sync(booking,slots[0])==expected
    assert service.events().update.called
    booking.status="cancelled"
    adapter.sync(booking,slots[0])
    assert service.events().delete.called
    service.events().delete().execute.side_effect=HttpError(SimpleNamespace(status=404,reason="Missing"),b"{}")
    assert adapter.sync(booking,slots[0])==expected
    service.events().delete().execute.side_effect=HttpError(SimpleNamespace(status=500,reason="Failure"),b"{}")
...
def test_optional_model_forced_output_and_budget(monkeypatch):
    ...
    def complete(**kwargs):
        calls.append(kwargs)
        arguments='{"intent":"book"}' if kwargs["tool_choice"]["function"]["name"]=="choose_intent" else '{"clarity":5,"tone":4}'
        return SimpleNamespace(choices=[SimpleNamespace(message=SimpleNamespace(tool_calls=[SimpleNamespace(function=SimpleNamespace(arguments=arguments))]))])
