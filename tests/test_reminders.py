...
async def test_reminder_step_actually_ran(db,clinic):
    ...
    def headers():
        ...
        nonce=uuid7()
        return {"X-Timestamp":timestamp,"X-Nonce":nonce,"X-Signature":signed(timestamp,nonce,"local-reminder-secret")}
    ...
    with Session(db) as session:
        ...
        attempts=session.exec(select(ReminderAttempt).where(ReminderAttempt.clinic_id==c.id)).all()
...
async def test_empty_due_query_is_still_a_run(db):
    ...
    nonce=uuid7()
    async with httpx.AsyncClient(transport=httpx.ASGITransport(app=create_app(db=db)),base_url="http://test") as client:
        assert (await client.post("/internal/reminders",headers={"X-Timestamp":timestamp,"X-Nonce":nonce,"X-Signature":signed(timestamp,nonce,"local-reminder-secret")})).status_code==200
    with Session(db) as session:
        assert session.exec(select(ReminderRun).where(ReminderRun.nonce==nonce)).one().query_executed
