    ...
    for name,args in cases:
        with Session(db) as session:
            with pytest.raises(Denied):
                invoke(session,a.id,conv_a.wa_id,conv_b.id if name in {"escalate_to_human","refuse_out_of_scope"} else conv_a.id,name,args)
            checks.append(True)
    for name in ["check_availability","book_slot","reschedule_booking","cancel_booking","get_my_bookings","escalate_to_human","refuse_out_of_scope"]:
        with Session(db) as session:
            with pytest.raises(Denied):
                invoke(session,b.id,conv_b.wa_id,conv_a.id,name,ToolArgs())
            checks.append(True)
    assert tenant_gate(checks)
    evidence(db,"tenant",{"attempts":len(checks),"explicitly_refused":sum(checks),"passed":True})
