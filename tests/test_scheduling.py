...
async def test_fifty_concurrent_claims(db, clinic):
    ...
    async with httpx.AsyncClient(transport=httpx.ASGITransport(app=app), base_url="http://test") as client:
        ...
        results = await asyncio.gather(*[client.post("/ops/claim", headers={"Authorization": "Bearer local-ops-token"}, json={"clinic_id": c.id, "wa_id": str(15550000000+i), "slot_id": slots[0].id}) for i in range(50)])
        elapsed = time.perf_counter()-started
    codes = [r.status_code for r in results]
    with Session(db) as session:
        count = len(session.exec(select(Booking).where(Booking.clinic_id == c.id, Booking.slot_id == slots[0].id)).all())
    measured = {"requests": 50, "confirmed": codes.count(200), "rejected": codes.count(409), "errors": sum(x not in (200,409) for x in codes), "booking_rows": count, "duration_seconds": round(elapsed,6), "under_two_seconds": elapsed < 2, "pool": "warm", "transport": "HTTP ASGI client to real Postgres"}
...
def test_dst(db):
    ...
    evidence(db,"dst",{"timezone":"America/New_York","ordinary_slots":len(ordinary),"spring_slots":len(spring),"fall_slots":len(fall),"spring_date":"2026-03-08","fall_date":"2026-11-01"})
...
def test_atomic_move_and_cancel(db, clinic):
    ...
    with Session(db) as session:
        ...
        second=claim(session,c.id,"15550002222",slots[1].id)
        identity=first.id
        session.commit()
    with Session(db) as session:
        with pytest.raises(Unavailable):
            claim(session,c.id,"15550001111",slots[1].id,own_booking(session,c.id,"15550001111",identity))
        session.rollback()
    with Session(db) as session:
        assert session.get(Booking,identity).slot_id==slots[0].id
        moved=claim(session,c.id,"15550001111",slots[2].id,own_booking(session,c.id,"15550001111",identity))
...
def test_overlapping_services_and_notice(db,clinic):
    ...
    with Session(db) as session:
        overlap=Slot(clinic_id=c.id,provider_id=p.id,service_id=services[1].id,starts_at=slots[0].starts_at,ends_at=slots[0].ends_at)
        session.add(overlap)
        session.commit()
