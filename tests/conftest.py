...
from frontdesk_core.models import Clinic, Conversation, Evidence, Provider, ProviderService, Service, Slot
...
def make_clinic(session, phone=None):
    ...
    clinic = Clinic(name="Fictional test clinic", whatsapp_phone_number_id=phone or uuid7(), business_hours={str(i): [0, 24] for i in range(7)})
    session.add(clinic)
    session.flush()
    provider = Provider(clinic_id=clinic.id, name="Avery Lane")
    services = [Service(clinic_id=clinic.id, name=name) for name in ["Welcome visit", "Routine visit", "Extended visit"]]
    ...
    for i, service in enumerate(services):
        session.add(ProviderService(clinic_id=clinic.id, provider_id=provider.id, service_id=service.id))
        for j in range(3):
            start = (now() + timedelta(days=2, hours=i * 3 + j)).replace(second=0, microsecond=0)
            slot = Slot(clinic_id=clinic.id, provider_id=provider.id, service_id=service.id, starts_at=start, ends_at=start+timedelta(minutes=30))
            session.add(slot)
            slots.append(slot)
