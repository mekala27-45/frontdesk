...
def seed(session: Session, phone: str = "demo-phone", name: str = "Ridgeview Family and Wellness Clinic") -> Clinic:
    ...
    session.add(clinic)
    session.flush()
    services = [Service(clinic_id=clinic.id, name=name, duration_minutes=duration, buffer_minutes=buffer) for name, duration, buffer in [("Welcome visit", 30, 0), ("Routine visit", 20, 10), ("Extended visit", 45, 15)]]
