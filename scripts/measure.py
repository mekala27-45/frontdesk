...
def main() -> None:
    ...
    subprocess.run(
        ...
        env={**os.environ, "PUBLISH_EVIDENCE": "1"},
        check=True,
    )
    database = engine()
    ...
    with Session(database) as session:
        quality = {
            ...
            "covered_statements": coverage["totals"]["covered_lines"],
            "total_statements": coverage["totals"]["num_statements"],
    ...
    for scenario in json.loads(values["redteam"])["scenarios"]:
        (replay_dir / (scenario["id"] + ".md")).write_text(
            "# "
            + scenario["id"]
