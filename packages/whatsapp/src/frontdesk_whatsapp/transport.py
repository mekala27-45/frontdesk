...
class MetaTransport:
    ...
    def send(self, payload: dict[str, Any], phone_id: str) -> str:
        response = self.client.post(f"https://graph.facebook.com/{self.settings.meta_graph_version}/{phone_id}/messages", headers={"Authorization": "Bearer " + self.settings.meta_token}, json=payload)
...
def dispatch(db: Engine, transport: WhatsAppTransport, clinic: str, phone_id: str) -> int:
    ...
    while True:
        with Session(db) as session:
            item = session.exec(select(Outbox).where(Outbox.clinic_id == clinic, Outbox.status == "pending").with_for_update(skip_locked=True)).first()
            if item is None:
                break
            item.status = "sending"
            session.add(item)
        ...
        with Session(db) as session:
            item = session.exec(select(Outbox).where(Outbox.id == item_id, Outbox.clinic_id == clinic)).one()
