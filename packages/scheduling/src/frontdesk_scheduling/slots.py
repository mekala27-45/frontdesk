...
def intervals(
    day: date, timezone: str, opening: int, closing: int, minutes: int
    ...
    for provider in session.exec(select(Provider).where(Provider.clinic_id == clinic_id)).all():
        links = session.exec(
            select(ProviderService).where(
                ProviderService.clinic_id == clinic_id, ProviderService.provider_id == provider.id
            )
        ).all()
        for link in links:
            service = session.exec(
                select(Service).where(Service.id == link.service_id, Service.clinic_id == clinic_id)
            ).one()
            for offset in range(clinic.booking_horizon_days):
                day = at.astimezone(ZoneInfo(clinic.timezone)).date() + timedelta(days=offset)
                hours = (provider.working_hours_override or clinic.business_hours).get(
                    str(day.weekday())
                )
                if not hours:
                    continue
