...
def own_booking(session: Session, clinic: str, owner: str, booking_id: str) -> Booking:
    booking = session.exec(
        select(Booking)
        .where(
            Booking.id == booking_id, Booking.clinic_id == clinic, Booking.patient_wa_id == owner
        )
        .with_for_update()
    ).first()
    if booking is None:
...
def claim(
    session: Session, clinic: str, owner: str, slot_id: str, booking: Booking | None = None
) -> Booking:
    ...
    for provider_id in sorted(provider_ids):
        provider = session.exec(
            select(Provider)
            .where(Provider.id == provider_id, Provider.clinic_id == clinic)
    ...
    slot = session.exec(
        select(Slot)
        .where(Slot.id == slot_id, Slot.clinic_id == clinic, Slot.status == "open")
        .with_for_update(skip_locked=True)
    ...
    if slot.starts_at < now() + timedelta(
        minutes=rules.min_notice_minutes
    ) or slot.starts_at > now() + timedelta(days=rules.booking_horizon_days):
    ...
    if booking is None:
        booking = Booking(
            clinic_id=clinic, slot_id=slot.id, patient_wa_id=owner, service_id=slot.service_id
        )
    else:
...
def queue_calendar(session: Session, booking: Booking) -> None:
    job = session.exec(
        select(CalendarJob).where(
            CalendarJob.clinic_id == booking.clinic_id, CalendarJob.booking_id == booking.id
        )
    ).first()
    if job:
        job.status = "pending"
...
def cancel(session: Session, clinic: str, owner: str, booking_id: str) -> Booking:
    ...
    session.exec(
        select(Provider)
        .where(Provider.id == slot.provider_id, Provider.clinic_id == clinic)
        .with_for_update()
    ).one()
    if booking.status == "confirmed":
