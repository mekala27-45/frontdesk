...
def run(db: Engine, timestamp: str, nonce: str, supplied: str, secret: str) -> dict[str, int]:
    if (
        not nonce
        or len(nonce) > 80
        or abs(now().timestamp() - float(timestamp)) > 300
        or not hmac.compare_digest(signed(timestamp, nonce, secret), supplied)
    ):
    ...
    with Session(db) as session:
        ...
        due = session.exec(
            ...
            .where(
                ...
                Slot.clinic_id == Booking.clinic_id,
                Slot.starts_at > now(),
                Slot.starts_at <= now() + timedelta(hours=24),
            )
        ).all()
            ...
            for threshold in (24, 2):
                ...
                existing = session.exec(
                    select(ReminderAttempt).where(
                        ReminderAttempt.clinic_id == booking.clinic_id,
                        ReminderAttempt.booking_id == booking.id,
                        ReminderAttempt.hours_before == threshold,
                    )
                ).first()
                if existing:
                    continue
                conversation = session.exec(
                    select(Conversation).where(
                        Conversation.clinic_id == booking.clinic_id,
                        Conversation.wa_id == booking.patient_wa_id,
                    )
                ).first()
                ...
                session.add(
                    Outbox(
                        ...
                        message_id="reminder:" + row.id,
                        payload=template_message(booking.patient_wa_id),
                    )
                )
                queued += 1
        session.add(attempt)
