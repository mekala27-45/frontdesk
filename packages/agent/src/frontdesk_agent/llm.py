...
class LiteLLMPolicy:
    ...
    def plan(self, cues: list[Intent]) -> Plan:
        ...
        messages = [
            {
                "role": "system",
                "content": "Choose one logistics intent from the supplied closed vocabulary. Call choose_intent.",
            },
        ...
        prompt_cost, output_cost = litellm.cost_per_token(
            model=self.model, prompt_tokens=4096, completion_tokens=128
        )
        ...
        with self.lock:
            if self.db is not None:
                with Session(self.db) as session:
                    ...
                    row = session.get(Evidence, key) or Evidence(
                        name=key, value={"reserved_usd": 0.0}
                    )
            ...
            tools=[
                {
                    ...
                    "function": {
                        "name": "choose_intent",
                        "description": "Select one allowed logistics intent",
                        "parameters": Plan.model_json_schema(),
                    },
                }
            ],
            tool_choice={"type": "function", "function": {"name": "choose_intent"}},
            max_tokens=128,
            temperature=0,
        )
