...
def respond(
    ...
    if category:
        ...
        call(
            "escalate_to_human" if category == "emergency" else "refuse_out_of_scope",
    ...
    if convo.status == "escalated":
        return (
            text_message(
                owner, "Your conversation is with the front desk. Please wait for their reply."
            ),
            calls,
            "handoff_wait",
        )
    ...
    if normalized in {"yes", "confirm"} and state.get("pending_slot"):
        ...
        result = call(
            name, ToolArgs(slot_id=state["pending_slot"], booking_id=state.get("booking_id"))
        )
        convo.state = {}
        reply = (
            "Your appointment is confirmed."
            if result.result.ok
            else "That time is no longer available. Please choose another."
        )
        return (
            buttons_message(
                owner,
    ...
    if normalized.startswith("slot:"):
        ...
        if selected not in state.get("offered_slots", []):
            ...
            return (
                text_message(owner, "Please choose one of the offered times."),
                calls,
                "invalid_choice",
            )
        ...
        return (
            buttons_message(
                owner,
                "Confirm this appointment time?",
                [("confirm", "Confirm"), ("no", "Start over"), ("human", "Talk to a person")],
            ),
            calls,
            "confirm_prompt",
        )
    if normalized.startswith("booking:"):
        ...
        if state.get("intent") == "cancel":
            ...
            return (
                text_message(
                    ...
                    if result.result.ok
                    else "Unable to cancel this appointment.",
                ),
                calls,
                "cancel",
            )
        ...
        state = {
            ...
            if re.search(r"reschedule|move", normalized)
            else "view",
            "offered_bookings": result.result.ids,
        }
        convo.state = state
        if state["intent"] == "view":
            return (
                text_message(
                    owner, "Your active appointment references: " + ", ".join(result.result.ids)
                ),
                calls,
                "bookings",
            )
        return (
            list_message(
                ...
                [
                    {
                        "id": "booking:" + key,
                        "title": "Appointment " + str(i + 1),
                        "description": key,
                    }
                    for i, key in enumerate(result.result.ids[:10])
                ],
            ),
            calls,
            "booking_choice",
    ...
    chosen = next(
        (
            ...
            if normalized == "service:" + service.id or service.name.lower() in normalized
        ),
        None,
    )
    if chosen:
    ...
    if date_match:
        try:
            requested = datetime.fromisoformat(date_match[0]).replace(
                tzinfo=ZoneInfo(clinic.timezone)
            )
            ...
            convo.state = {}
            return (
                text_message(owner, "Please choose a valid date within the booking horizon."),
                calls,
                "invalid_date",
            )
    if state.get("service_id"):
        ...
        if rows:
            return (
                list_message(
                    owner, "Choose an available time. Times use the clinic timezone.", rows
                ),
                calls,
                "availability",
            )
        return (
            text_message(
                owner,
                "No times are available for that request. Try another date or talk to a person.",
            ),
            calls,
            "no_availability",
        )
    if (
        re.search(
            r"book|appointment|visit|hello|hi\b|schedule|available|morning|afternoon|tomorrow",
            normalized,
        )
        or normalized in {"", "no", "start over", "back"}
        ...
        return (
            list_message(
                owner,
                "Welcome to fictional Ridgeview. Booking logistics only. Please choose a service.",
                [{"id": "service:" + service.id, "title": service.name} for service in services],
            ),
            calls,
            "service_prompt",
        )
...
def ingest(
    ...
    with Session(db) as session:
        clinic = session.exec(
            select(Clinic).where(Clinic.whatsapp_phone_number_id == phone_id)
        ).first()
        if clinic is None:
            raise Denied("Unknown phone identity")
        lock_conversation(session, clinic.id, owner)
        duplicate = session.exec(
            select(Message).where(Message.wamid == wamid, Message.clinic_id == clinic.id)
        ).first()
        if duplicate:
            return {
                "duplicate": True,
        ...
        session.add(convo)
        session.add(
            Outbox(clinic_id=clinic.id, conversation_id=convo.id, message_id=wamid, payload=payload)
        )
        session.flush()
        session.add(
            Message(
                ...
                content_summary=str(
                    ...
                    or payload.get("interactive", {}).get("body", {}).get("text")
