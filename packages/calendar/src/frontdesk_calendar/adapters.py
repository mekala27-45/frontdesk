...
class GoogleCalendarAdapter:
    ...
    def sync(self, booking: Booking, slot: Slot) -> str:
        ...
        body = {
            "id": event_id,
            "summary": "Fictional clinic appointment",
            "start": {"dateTime": slot.starts_at.isoformat()},
            "end": {"dateTime": slot.ends_at.isoformat()},
            "description": "Demo logistics only. Reference " + booking.id,
        }
        try:
...
def sync_pending(db: Engine, adapter: CalendarAdapter) -> int:
    ...
    with Session(db) as session:
        jobs = session.exec(
            select(CalendarJob)
            .where(CalendarJob.status == "pending")
            .with_for_update(skip_locked=True)
        ).all()
        for job in jobs:
            booking = session.exec(
                select(Booking).where(
                    Booking.id == job.booking_id, Booking.clinic_id == job.clinic_id
                )
            ).one()
            slot = session.exec(
                select(Slot).where(Slot.id == booking.slot_id, Slot.clinic_id == job.clinic_id)
            ).one()
            try:
